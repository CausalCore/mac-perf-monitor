...
from mac_monitor.detector.system import (
    get_cpu_metrics,
    get_memory_metrics,
    get_disk_io_metrics,
)
...
from mac_monitor.actions.executor import (
    kill_process,
    remove_login_item,
    stop_brew_service,
)
...
def generate_action_plan():
    ...
    for p in top_procs:
        ...
        score, reason, action = evaluate_process_action(
            p, p_trends.get("cpu_growth", False), p_trends.get("ram_growth", False)
        )
        if action:
...
def cmd_optimize(args):
    ...
    console.print(
        "[bold cyan]Analyzing System Telemetry and Time-Series Data...[/bold cyan]"
    )
    ...
    if not actions:
        console.print(
            Panel(
                "[green]System is fully optimized. No critical anomalies detected.[/green]",
    ...
    for a in actions:
        ...
        table.add_row(
            conf, a["type"].upper(), a.get("name", str(a.get("pid"))), a["reason"]
        )
...
def cmd_kill(args):
    ...
    if p_info:
        ...
        if score == 0:
            console.print(
                f"[bold red]BLOCKED:[/bold red] PID {pid} ({p_info['name']}) is a protected system process."
            )
            return
...
def main():
    parser = argparse.ArgumentParser(
        description="macOS Local Autonomous System Controller"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # optimize command
    parser_opt = subparsers.add_parser(
        "optimize", help="Run the AI optimization engine"
    )
    parser_opt.add_argument(
        "--plan",
        action="store_true",
        dest="plan_only",
        help="Only show the decision plan (default if no other flags)",
    )
    parser_opt.add_argument(
        "--dry", action="store_true", help="Run the plan but simulate actions"
    )
    parser_opt.add_argument(
        "--apply",
        action="store_true",
        help="Execute the plan immediately (requires YES confirm per action)",
    )
    ...
    parser_kill.add_argument(
        "--force",
        action="store_true",
        help="Skip confirmation (still respects whitelist)",
    )
