...
def run_network_sentinel():
    console.print(
        "[bold cyan]Scanning Active Network Connections (Privacy Sentinel)...[/bold cyan]\n"
    )
    ...
    if not conns:
        console.print(
            Panel(
                "[yellow]Unable to read active connections without sudo, or no established connections found.[/yellow]"
            )
        )
        return
