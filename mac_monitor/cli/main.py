...
def run_analysis(duration_sec=10):
    ...
    with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
        progress.add_task(description=f"Running Multi-Baseline Diagnostics (Micro-Buffer: {duration_sec}s)...", total=None)
...
def print_causality_graph(report):
    ...
    if not roots:
        console.print("[green]No significant bottlenecks detected. System is healthy.[/green]")
        return
    ...
    for idx, cause in enumerate(roots):
        ...
        console.print(f" {prefix} [bold yellow]Bottleneck[/bold yellow] (Score: {c_score:.1f})")
...
def cmd_analyze(args):
    ...
    color = "red" if report["score"] > 60 else "yellow" if report["score"] > 30 else "green"
    ...
    if recs:
        ...
        for r in recs:
            console.print(f"  - [dim]{r['cause']} (Seen {r['frequency']} times recently)[/dim]")

def cmd_boost(args):
    console.print("[bold]macOS Behavioral Causality Intelligence Engine - Boost Mode[/bold]")
    report = run_analysis(10)
...
def main():
    parser = argparse.ArgumentParser(description="macOS Behavioral Causality Intelligence Engine")
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Analyze
    parser_analyze = subparsers.add_parser("analyze", help="Run full causality diagnostics (DAG + Root Cause)")

    # Dashboard
    parser_dash = subparsers.add_parser("dashboard", help="Launch the Live TUI Dashboard")

    # Tray
    parser_tray = subparsers.add_parser("tray", help="Launch macOS Native Menu Bar App")

    # Boost
    parser_boost = subparsers.add_parser("boost", help="Generate and apply behavior optimizations")
    parser_boost.add_argument("--plan", action="store_true", help="Show optimization plan")
    parser_boost.add_argument("--apply", action="store_true", help="Show terminal commands to execute the plan")

    # Report
    parser_report = subparsers.add_parser("report", help="Generate a weekly HTML health report")

    # Battery
    parser_battery = subparsers.add_parser("battery", help="Detect background battery drainers")
    
    # Focus
    parser_focus = subparsers.add_parser("focus", help="Enable Focus Mode (Pause background syncs)")
    parser_unfocus = subparsers.add_parser("unfocus", help="Disable Focus Mode")
    
    # Clean
    parser_clean = subparsers.add_parser("clean", help="Find hidden storage bloat and caches")
    
    # Share
    parser_share = subparsers.add_parser("share", help="Generate a shareable health score ASCII art")

    # Network
    parser_network = subparsers.add_parser("network", help="Scan for network privacy hogs")
    
    # Widget
    parser_widget = subparsers.add_parser("widget", help="Launch transparent Desktop Widget Proxy")
    
    # Chat
    parser_chat = subparsers.add_parser("chat", help="Chat with Local AI about system health")
    parser_chat.add_argument("prompt", nargs="?", default=None, help="Your question to the AI")

    # Autostart
    parser_autostart = subparsers.add_parser("autostart", help="Find and disable startup bloat (LaunchAgents)")
    
    # Uninstall
    parser_uninstall = subparsers.add_parser("uninstall", help="Deep clean app remnants")
    parser_uninstall.add_argument("app", help="Name of the app to eradicate")
    
    # Free RAM
    parser_freeram = subparsers.add_parser("free-ram", help="Force flush RAM disk cache")
