...
def run_autostart_scan():
    console.print(
        "[bold cyan]Scanning for Startup Bloat (LaunchAgents)...[/bold cyan]\n"
    )

    paths_to_scan = ["~/Library/LaunchAgents"]
    ...
    for base_path in paths_to_scan:
        ...
        if os.path.exists(expanded):
            try:
                ...
                console.print(
                    f"[bold red]Permission Denied:[/bold red] Terminal needs 'Full Disk Access' to scan {expanded}"
                )
                return
    ...
    if found_any:
        ...
        console.print(
            "[green]No user-level startup bloat found in ~/Library/LaunchAgents.[/green]"
        )
