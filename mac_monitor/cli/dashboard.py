...
def update_dag_panel(report):
    ...
    if not roots:
        return Panel(Align.center("[green]\n\nSystem Healthy\nNo bottlenecks detected.[/green]"), title="Causality DAG")
    ...
    for idx, cause in enumerate(roots):
        ...
        text += f" {prefix} [bold yellow]Bottleneck[/bold yellow] (Score: {c_score:.1f})\n"
...
def run_dashboard():
    ...
    layout["header"].update(Panel(Align.center("[bold cyan]macOS Behavioral Causality Engine - Live TUI[/bold cyan]")))
