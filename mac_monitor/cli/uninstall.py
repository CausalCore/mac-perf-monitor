...
def run_uninstall(app_name):
    console.print(
        f"[bold cyan]Scanning for deep remnants of '{app_name}'...[/bold cyan]\n"
    )
    ...
    for line in lines:
        if line.startswith(home_dir) and (
            "Library/Application Support" in line
            or "Library/Caches" in line
            or "Library/Preferences" in line
            or "Library/Logs" in line
            or "Library/Containers" in line
        ):
            targets.append(line)

    if not targets:
        console.print(
            Panel(f"[green]No hidden remnants found for '{app_name}'.[/green]")
        )
        return
